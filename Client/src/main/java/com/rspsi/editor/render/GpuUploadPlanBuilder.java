            ...
            if (!commands.isEmpty()) {
                for (GpuDrawCommand cmd : commands) {
                    ...
                    cmdBuffer.putInt(cmd.tile().plane())
                            ...
                            .putInt(cmd.sceneObjectIdentity().shape())
                            .putInt(cmd.sceneObjectIdentity().rotation())
                            .putInt(cmd.sceneObjectIdentity().authoredPlane())
                            .putInt(cmd.sceneObjectIdentity().anchorX())
                            .putInt(cmd.sceneObjectIdentity().anchorY())
                            .putInt(cmd.sceneObjectIdentity().footprintWidth())
                            .putInt(cmd.sceneObjectIdentity().footprintLength())
                            .putInt(cmd.sceneObjectIdentity().occurrence())
                    ...
                    for (int renderableIndex = 0;
                         renderableIndex < cmd.clientRenderableBounds().size();
                         renderableIndex++) {
                        ClientModelBounds bounds = cmd.clientRenderableBounds().get(renderableIndex);
                        ClientRenderablePlacement placement =
                                cmd.clientRenderablePlacements().get(renderableIndex);
                        ...
                        cmdBuffer.putInt(bounds.present() ? 1 : 0)
                                ...
                                .putInt(placement.offsetX())
                                .putInt(placement.offsetZ());
                    }
                    cmdBuffer.flip();
