...
|---|---|---|---|
| Actual 3D presentation | `Scene.draw`, camera projection, tile traversal, and `SceneUploader` | `EmbeddedOpenGlViewport`/`OpenGlSceneRenderer` now consume the neutral upload plan; `CanonicalSceneViewport` remains a top-down semantic JavaFX reference adapter and `SceneGraph` remains compatibility infrastructure | Continue parity work in the renderer-independent packet and OpenGL backend; do not make JavaFX Canvas or the legacy renderer define the neutral scene |
