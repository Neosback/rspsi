...
| AtlantaFX | [mkpaz/atlantafx](https://github.com/mkpaz/atlantafx) | `production-candidate` | CSS-first Primer Dark theme foundation and custom RSPSi tokens | MIT declared upstream; pin and preserve notice | Use only in `Editor`; keep neutral packages JavaFX-free |
| Ikonli | [kordamp/ikonli](https://github.com/kordamp/ikonli) | `production-candidate` | JavaFX Material Design icons through the semantic `StudioIcon` registry | Apache-2.0 declared upstream; verify selected pack version | Use `ikonli-javafx` and `ikonli-materialdesign2-pack`; no new raw glyph utilities |
| ControlsFX | [controlsfx/controlsfx](https://github.com/controlsfx/controlsfx) | `production-candidate` | Mature JavaFX utility controls retained during shell migration | BSD-3-Clause declared upstream; preserve notices | Keep as an optional utility layer, not the layout system |
