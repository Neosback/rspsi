...
| Neutral definitions | implemented-unverified | Object/floor/texture/collision views plus lazy OpenRune model metadata now cross the adapter; `DefinitionAssetRepository` provides neutral searchable asset descriptors and an optional `SymbolicNameProvider` seam for RSCM/GameVal keys, while backend wiring and real-cache fixture parity remain |
