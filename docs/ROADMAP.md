...
| Neutral world validation | implemented-unverified | `WorldValidator` reports broken shared edges, unsupported OSRS map values, duplicate/invalid objects using canonical shape semantics, missing definitions, and definition-backed footprint bounds; the controlled workspace now exposes live Validation diagnostics with a neutral definition adapter, while full parity rules remain |
