...
| Route and line-of-sight preview | implemented-unverified | Neutral bounded `RouteFinder`, `LineValidator`, `Reachability`, and `RoutePreviewService` provide collision-aware routes, OpenRune-compatible swept footprint checks for square actors, edge-aware rectangular line-of-sight and line-of-walk validation, destination location semantics, explicit optional route-blocker semantics, corner-cutting protection, projectile traces, object-footprint reach previews, and safe map-edge handling; the controlled JavaFX viewport now renders immutable route/LOS/reach results with start/target markers and exposes Route/LOS/Reach controls; broader OpenRune-Server route/reach parity remains |
