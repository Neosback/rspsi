...
| First-party terrain tools | implemented-unverified | Command-backed overlay, vertex-aware raise/lower with radius/falloff, ramp, bilinear height sampling, tile-flag, flatten, and neighbour-aware smoothing brushes are tested; canonical JavaFX pointer routing and tool overlays are now available, and the constrained OSRS rail exposes core terrain tools plus collapsed ramp/flag tools while preserving the legacy rail; broader interactive smoke coverage and legacy migration remain |
