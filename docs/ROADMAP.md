...
| Canonical collision map | implemented-unverified | RSPSi-owned flags/map plus `OsrsCollisionBuilder` consume canonical object categories for bridge-aware terrain masks, roof semantics, rotated footprint collision, wall-decor behavior, OpenRune route-blocker flags, and decoded collision-inspector snapshots including route-blocked directions; live build-240 region `(50,50)` now produces 3,983 non-empty collision tiles from 4,726 object projections, while route/LOS parity and complete definition fixtures remain |
