...
| Neutral cache boundary | in-progress | `CacheStore` facade introduced; the client/resource path now routes regular and named sprite reads through byte-oriented `Cache.readFile(...)`/`readNamedFile(...)` methods, while deprecated index access remains only for compatibility loaders; migrate remaining consumers incrementally |
