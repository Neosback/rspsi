...
| Autosave | implemented-unverified | Legacy `AutoSaveJob` remains as a compatibility path; neutral `SessionAutosaveStore` now writes atomic versioned project snapshots to the canonical `ProjectLayout.sessionAutosaveFile()` path and restores terrain, flags, and objects; `SessionAutosaveCoordinator` now binds edit/undo/redo/save-marker transitions to that store and `OsrsStudioProject.attachAutosave(...)` verifies project identity, while JavaFX scheduling/recovery UI remains |
