...
| OpenRune backend | in-progress | 2.4.19 compatibility spike and neutral OSRS region decoder are isolated behind `CacheStore`; `CacheStoreFactory.openOsrs(Path)` and the compatibility facade now route modern OSRS reads through OpenRune, while 317 remains explicitly legacy; external revision-6 named and live build-240 numeric terrain/location verification passes, staged modern output persists through the explicit Displee adapter, and the OpenRune `CacheDelegate` writer has deterministic byte-and-semantic round-trip coverage on a fresh cache plus encoder poison-byte/shape/overlay fixes; the normal source reader remains read-only and the 2.4.19→3.0.2 upgrade stays pinned behind the [`FILESTORE_CAPABILITY_AUDIT.md`](FILESTORE_CAPABILITY_AUDIT.md) runbook |
...
The next gate is packet construction from the existing `RenderSceneBuilder`:
final terrain colors/UVs, transformed model packets, scene-layer ordering,
occluder inputs, and fingerprints. Only after those fixtures pass should an
OpenGL 3.3 adapter be added. GPU rendering is a packet consumer, not a second
OSRS decoder or a pre-rendered map image.
