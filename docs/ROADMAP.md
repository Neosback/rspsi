...
| Neutral cache boundary | in-progress | `CacheStore` facade introduced; regular resources, named sprite reads, and the legacy byte accessor now route through byte-oriented cache methods, while deprecated index access remains only for compatibility loaders; migrate remaining consumers incrementally |
