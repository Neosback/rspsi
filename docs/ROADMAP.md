...
| Canonical collision map | implemented-unverified | RSPSi-owned flags/map plus `OsrsCollisionBuilder` provide bridge-aware terrain masks, roof semantics, rotated footprint collision, wall/ground categories, and a decoded collision-inspector snapshot; route/LOS parity and complete definition fixtures remain |
