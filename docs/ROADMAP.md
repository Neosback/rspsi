...
| Neutral definitions | implemented-unverified | Object/floor/texture/collision views plus lazy OpenRune model metadata, map-scene IDs, optional immutable `MapSceneSpriteView` assets, and object appearance metadata (animation, contouring, transforms, recolor/retexture pairs) now cross the neutral boundary; `DefinitionAssetRepository` and `OpenRuneSymbolicNameProvider` expose optional neutral RSCM/GameVal keys and rich neutral asset property summaries, while OpenRune graphics-defaults sprite-group loading, mapping-file lifecycle, and real-cache fixture parity remain |
