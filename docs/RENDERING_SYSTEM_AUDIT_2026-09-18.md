...
**Frontend status note (added after this document was written):**
`docs/ROADMAP.md` is the authoritative execution plan. The native GLFW +
Dear ImGui shell's `NativeSceneViewport` is the production scene viewport;
the legacy JavaFX `EmbeddedOpenGlViewport`/`CanonicalSceneViewport` referenced
throughout this document as "the embedded surface"/"production 3D backend"
are transitional/compatibility surfaces only, not reachable from `main()`.
The `OpenGlSceneRenderer` class this document describes is the same class
either way (`com.rspsi.renderer.opengl`, since moved to match its package
from a leftover `com.rspsi.ui.workspace` file location) - the renderer
behavior/correctness content below remains accurate; only the "which
viewport hosts it in production" framing predates the native shell becoming
primary.
...
| scene-tile/GPU upload packets | `GpuScenePacketBuilder` assembles stable world-tile order, terrain/model packets, explicit terrain/object layer order, opaque/transparent model partitions, bridge/effective-plane visibility, raw tile flags, world/region/chunk identity, explicit definition-backed occluders, model-clipped wall occluder inputs, texture resources, and deterministic fingerprints; `GpuUploadPlanBuilder` now flattens those packets into world-space vertices, indices, material references, per-command face bias, deterministically merged contiguous occluder planes, and ordered draw commands consumed by both native and reference backends; `GpuPriority` now applies only the true client face-bias depth policy, matching RuneLite's real vertex shader (`screenPos.z += float(bias) / 128.0`); the synthetic priority-to-NDC band term was removed as a correctness bug (it had no RuneLite counterpart and could cause incorrect z-fighting between differently-prioritized opaque faces). Exactly-coplanar opaque priority ties are now resolved by priority-descending draw order in both `SoftwareSceneRenderer` and `OpenGlSceneRenderer`, matching RuneLite's "priority affects order, never depth" model | partial: revision-specific plane-cull/force-visible interpretation, exact mask activation/traversal, explicit `DEFAULT`/`SORTED`/`SORTED_NO_DEPTH`/`UNSORTED`/`UNSORTED_NO_DEPTH` render modes, and advanced material/shadow channels remain |
...
| production 3D backend | The native GLFW/Dear ImGui shell's `NativeSceneViewport` + `OpenGlSceneRenderer` (`com.rspsi.renderer.opengl`, moved to match its package from a leftover `com.rspsi.ui.workspace` location) is the production consumer for opened OSRS projects, per `ROADMAP.md`; the legacy JavaFX `EmbeddedOpenGlViewport` is a transitional/compatibility surface only, not reachable from `main()`. The context requests OpenGL 3.3 core with MSAA now enabled by default (registry `0..8`, default `4`, clamped to `GL_MAX_SAMPLES` by `GlFramebuffer`), binds a VAO before shader validation, uses GLSL 330 and a core texture array, consumes the host's explicit client cycle, applies raw face bias with one `/128` scale in the shader (no synthetic priority band), gates geometry/texture upload on the plan's camera-independent fingerprint only via `GpuCommandVisibility` (camera movement no longer re-uploads GPU buffers or shatters merged draw commands near occluders), and uses the shared fog presentation path | partial: native surface and live cycle refresh are wired, but full client parity, picking, and cross-platform runtime fixtures remain; context failure now reports an actionable error instead of mounting JavaFX Canvas |
